...
namespace UnityEngine.Experimental.Input
{
    ...
    internal class NativeInputDeviceManager : INativeInputDeviceManager
    {
        ...
        public void Initialize(InputDeviceManager deviceManager, InputDeviceProfileManager profileManager)
        {
            ...
            // across reloads.
            for (int i = 0; i < m_DeviceManager.devices.Count; i++)
            {
                var device = m_DeviceManager.devices[i];
                if (device.nativeId == 0)
                    continue;

                for (var j = 0; j < m_NativeDevices.Count; j++)
                {
                    if (m_NativeDevices[j].deviceInfo.deviceId != device.nativeId)
                        continue;

                    m_NativeDevices[j] = new NativeDeviceRecord
                    {
                        device = device,
                        deviceInfo = m_NativeDevices[j].deviceInfo
                    };
