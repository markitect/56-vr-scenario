...
namespace UnityEngine.Experimental.Input
{
    ...
    internal class InputSystemManager : ScriptableObject
    {
        ...
        {
#if UNITY_EDITOR
            var inputSystemActiveByDefault = UnityEditor.EditorApplication.isPlaying && Application.isFocused;
#endif
            ...
            switch (currentUpdateType)
            {
...
#if UNITY_EDITOR
                    InputSystem.isActive = inputSystemActiveByDefault;
#endif
                ...
                case NativeInputUpdateType.BeginEditor:
                    InputSystem.isActive =
                        UnityEditor.EditorApplication.isPlaying
                        ? !Application.isFocused     // When game view is focused and we're playing, EditorWindows don't see input state change.
                        : true;     // When we're not playing, EditorWindows always see input state update.
                    if (InputSystem.isActive)
                        eventManager.handlerRoot.BeginUpdate();
                    break;
